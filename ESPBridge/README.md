...
| `Mjson` | `Mjson\r\n` | Установка режима, при котором после соединения и отправки данных возвращается содержащийся в ответе от сервера JSON-обьект, или `rFAIL_NOJSON:XXX`, где `XXX` - HTTP-код ответа. Возвращает `rOK_MODE_JSON\r\n`. |
| `Mjson` | `Mjson:YYYY\r\n` | Установка режима, при котором после соединения и отправки данных возвращается содержимое содержащегося в ответе от сервера JSON-обьекта по ключу `YYYY`, или `rFAIL_NOJSONENTRY:YYYY:XXX`, где `YYYY` - запрошенный ключ и `XXX` - HTTP-код ответа, или `rFAIL_NOJSON:XXX`, где `XXX` - HTTP-код ответа. Возвращает `rOK_MODE_JSON\r\n`. |
| `Mhttpcode` | `Mhttpcode\r\n` | Установка режима, при котором после соединения и отправки данных возвращается HTTP-код ответа сервера. Возвращает `rOK_MODE_JSON\r\n`. |
| `Mfull` | `Mfull\r\n` | Установка режима, при котором после соединения и отправки данных возвращается ответ сервера полностью. *Режим по умолчанию.* Возвращает `rOK_MODE_JSON\r\n`. |
