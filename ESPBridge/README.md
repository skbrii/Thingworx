...
| `Mjson` | `Mjson:YYYY\r\n` | Установка режима, при котором после соединения и отправки данных возвращается содержимое содержащегося в ответе от сервера JSON-обьекта по ключу `YYYY`, или `rFAIL_NO_JSON_ENTRY:YYYY:XXX`, где `YYYY` - запрошенный ключ и `XXX` - HTTP-код ответа, или `rFAIL_NO_JSON:XXX`, где `XXX` - HTTP-код ответа. Возвращает `rOK_MODE_JSON_ENTRY\r\n`. |
